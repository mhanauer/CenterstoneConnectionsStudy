...
Conn_Base = PHQ9_GAD7[c("IncomeWages.x", "LivingWhere.y", "HealthStatus.x", "Age.x", "EducationYears.x", "Gender.x", "DAUseIllegDrugsDays.x", "County.x", "GAD7Base", "PHQ9Base", "ArrestedDays.x", "ERPhysical.x", "ERMental.x", "ER_visit")]

names(Conn_Base) = c("IncomeWages", "LivingWhere", "HealthStatus", "Age", "EducationYears", "Gender", "DAUseIllegDrugsDays", "County", "GAD7Base", "PHQ9Base", "ArrestedDays", "ERPhysical", "ERMental", "ER_visit")
...
Conn_Base_complete$ArrestedDays = NULL
Conn_Base_complete$ERPhysical = NULL
Conn_Base_complete$ERMental = NULL
Conn_Base_complete$GAD7Base = NULL
#Conn_Base_complete$DAUseIllegDrugsDays = NULL
#Conn_Base_complete$HealthStatus = NULL
#Conn_Base_complete$EducationYears = NULL
```
Get descirptives
```{r}
...
summary(logit_model)
library(konfound)
konfound(logit_model, "Age")

logit_model_phq9 = glm(LivingWhere ~ IncomeWages +  HealthStatus + Age*PHQ9Base + EducationYears + Gender + DAUseIllegDrugsDays + County + PHQ9Base + ER_visit, data = Conn_Base_complete)
summary(logit_model)

logit_model_ER_visit = glm(LivingWhere ~ IncomeWages +  HealthStatus + Age*ER_visit + EducationYears + Gender + DAUseIllegDrugsDays + County + PHQ9Base + ER_visit, data = Conn_Base_complete)
summary(logit_model)
...
Try getting interaction effects
```{r}
y = as.matrix(Conn_Base_complete$LivingWhere)
x = as.matrix(subset(Conn_Base_complete, select = -LivingWhere))
head(x)

library(glmnet)
library(glinternet)
set.seed(1234)
cv.lasso <- cv.glmnet(x, y, alpha = 1, family = "binomial", type.measure = "auc", nfolds = 7)
exp(coef(cv.lasso, cv.lasso$lambda.min))
```
Test which ones are significant
```{r}
library(selectiveInference)
n = length(Conn_Base_complete)
lambda = cv.lasso$lambda.min
beta = coef(cv.lasso, s = cv.lasso$lambda.min/n)
p_value = fixedLassoInf(x, y, beta, lambda, family = "binomial")
p_value
```
...
```
Try interaction of lasso
```{r}
numLevels = c(2,1,1,2,2,1,2,1,1)
fit = glinternet.cv(x, y, numLevels, family="binomial", nFolds = 6)
coef(fit$glinternetFit)
```
Try caret
```{r}
Conn_Base_complete$LivingWhere = as.factor(Conn_Base_complete$LivingWhere)
library(caret)
inTrain = createDataPartition(y = Conn_Base_complete$LivingWhere, p = .75, list = FALSE)
training = Conn_Base_complete[inTrain,]
testing = Conn_Base_complete[-inTrain,]
```
Now get the model
```{r}
...
```{r}
set.seed(12345)
gbmFit1 <- train(LivingWhere ~ ., data = training, 
                 method = "gbm", 
