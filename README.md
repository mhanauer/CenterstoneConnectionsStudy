...
Checking to see if the data is missing at random.  Get rid of any data that would mess it up, which includes the dates, id's and arrested days which has almost all zeros.
```{r}
...
```
Center everything besides categorical, date, and count (centering count variables could result in negative and not whole numbers).  So create two data sets one for the categorical outcomes and one for the continious ones that will be centered.  Then recombine them into one data set overwritting the old one.
```{r}
